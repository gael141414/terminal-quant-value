...
def ejecutar_visor_gurus(ticker_input, res_is, res_bs, res_cf, res_val):
    ...
    ey = res_val.get('earnings_yield', 0) * 100 if res_val else 0
    ...
    per = res_val.get('per_asumido', 0) if res_val else 0
    crecimiento = res_val.get('crecimiento_sostenible', 0) * 100 if res_val else 0
    ...
    with col_p3:
        if per > 0 and crecimiento > 0:
            ...
            st.write("Datos insuficientes para el ratio PEG.")
    ...
    if "Crecimiento Ventas %" in res_is["ratios"].columns:
        ...
        if len(hist_ventas) >= 3:
            ...
            st.info("No hay suficientes datos para calcular la media de crecimiento a 3 años.")
    else:
